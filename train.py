...
if __name__ == "__main__":
    ...
    Init_Epoch = 0
    Freeze_Epoch = 50
    # Freeze_batch_size = 16  # 源代码的batch_size
    Freeze_batch_size = 8
    # ------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    # ------------------------------------------------------------------#
    # UnFreeze_Epoch      = 300      #源代码的总epoch
    UnFreeze_Epoch = 200
    # Unfreeze_batch_size = 8  # 源代码的unfreeze_batch_size
    ...
    # ------------------------------------------------------------------#
    # Init_lr = 1e-2      #源码代码的sgd初始学习率（bs=64的学习率），adam的Init_lr = 1e-3开始设置（bs=64的学习率）
    Init_lr = 10e-3       # 初始学习率变为原来4倍，在bs减半情况下，得到学习率为原来2倍,
    Min_lr = Init_lr * 0.01   #源码默认最小学习率是最大的0.01倍
    # Min_lr = Init_lr * 0.001     #尝试把它改的更小
    ...
    # ------------------------------------------------------------------#
    # optimizer_type = "sgd"    #源码默认采用sgd
    optimizer_type = "adam"
    momentum = 0.937
    #weight_decay是一个非常敏感的参数，一般很小很小
    weight_decay = 5e-4

    if optimizer_type=='adam':
        weight_decay=0
        momentum=0.9    #这是Adam默认的一阶动量系数
    # ------------------------------------------------------------------#
    ...
    # ------------------------------------------------------------------#
    # focal_loss          = False   #源代码默认关闭focal loss
    focal_loss = True
    focal_alpha = 0.25
    focal_gamma = 2
    # ------------------------------------------------------------------#
    ...
    eval_flag = True
    eval_period = 10
    # 自己添加的参数，表示计算mAP的方式，可以为'mAP0.5'或'cocomAP'
    mAP_mode = 'mAP0.5'
    ...
    train_annotation_path = 'fire_city_train.txt'
    val_annotation_path = 'fire_city_val.txt'
    ...
    # ------------------------------------------------------#
    #   创建yolo模型
    #   这里的pretrained加载的是backbone预训练权重
    # ------------------------------------------------------#
    model = YoloBody(anchors_mask, num_classes, backbone=backbone, pretrained=pretrained)
    if not pretrained:
        weights_init(model)

    # 如果设置了model_path，在这里加载整个网络的预训练权重，会覆盖掉之前加载的backbone预训练权重
    if model_path != '':
        # ------------------------------------------------------#
        #   权值文件请看README，百度网盘下载
        # ------------------------------------------------------#
    ...
    if local_rank == 0:
        show_config(
            ...
            model_path=model_path, input_shape=input_shape, Init_Epoch=Init_Epoch, \
            ...
            mosaic=mosaic, mosaic_prob=mosaic_prob, mixup=mixup, mixup_prob=mixup_prob, \
            focal_loss=focal_loss, focal_alpha=focal_alpha, focal_gamma=focal_gamma, \
            train_annotation_path=train_annotation_path,val_annotation_path=val_annotation_path
        )
        # ---------------------------------------------------------#
        #   总训练世代指的是遍历全部数据的总次数
        #   总训练步长指的是梯度下降的总次数 
    ...
    if True:
        ...
        # add_param_group是把新的待训练网络权重加入optimizer，默认参数与之前相同
        # 如果定义了不同的参数，那么该部分网络权重的相关参数使用新定义的值
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            # 注意yolo_loss是一个函数，这里把它传进去
            fit_one_epoch(model_train, model, yolo_loss, loss_history, eval_callback,
                          optimizer, epoch, epoch_step, epoch_step_val, batch_size,gen, gen_val,
